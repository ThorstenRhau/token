...
local ns_color_archive = lib.ns_color_archive
...
local apple_terminal_ansi_keys = {
  'ANSIBlackColor',
  'ANSIRedColor',
  'ANSIGreenColor',
  'ANSIYellowColor',
...
local function apple_terminal_color(lines, key, hex)
  lines[#lines + 1] = '    <key>' .. key .. '</key>'
  lines[#lines + 1] = '    <data>' .. ns_color_archive(hex) .. '</data>'
end

local function gen_apple_terminal(p, variant, term, appearance)
  local lines = {
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">',
    '<!-- Generated by token colorscheme. Do not edit manually. -->',
  ...
  for index, key in ipairs(apple_terminal_ansi_keys) do
    apple_terminal_color(lines, key, term[index - 1])
  end
  apple_terminal_color(lines, 'BackgroundColor', p.bg3)
  apple_terminal_color(lines, 'TextColor', p.fg0)
  apple_terminal_color(lines, 'TextBoldColor', p.fg0)
  apple_terminal_color(lines, 'CursorColor', p.fg0)
  apple_terminal_color(lines, 'SelectionColor', p.sel)

  -- Terminal.app has no cursor text colour key and paints the glyph on top of
  -- the cursor fill in the normal text colour, so a block cursor in fg0 hides
  -- the character under it. Type 1 is an underline, which keeps the cursor in
  -- the palette's own colour and the character readable. Terminal also has no
  -- selection text or link colour.
  lines[#lines + 1] = '    <key>CursorType</key>'
  lines[#lines + 1] = '    <integer>1</integer>'
  lines[#lines + 1] = '    <key>ProfileCurrentVersion</key>'
  lines[#lines + 1] = '    <real>2.09</real>'
  -- Terminal names an imported profile after the file, not after this key, so
  -- the two are kept identical: the key only matters if the dict is merged into
  -- com.apple.Terminal.plist directly.
  lines[#lines + 1] = '    <key>name</key>'
  lines[#lines + 1] = '    <string>' .. appearance.slug .. '-' .. variant .. '</string>'
  lines[#lines + 1] = '    <key>type</key>'
  lines[#lines + 1] = '    <string>Window Settings</string>'
  lines[#lines + 1] = '  </dict>'
  lines[#lines + 1] = '</plist>'
  lines[#lines + 1] = ''

  return {
    path = 'contrib/apple-terminal/' .. appearance.slug .. '-' .. variant .. '.terminal',
    content = table.concat(lines, '\n'),
  }
end
...
local function gen_xcode(p, variant, _term, appearance)
  ...
  if profile then
    ...
    local profile_colors = {
      ...
      ['xcode.syntax.number'] = (r.number or r.literal).fg,
...
local function obsidian_theme_block(p, variant, appearance)
  ...
  extend_lines(lines, {
    ...
    '  --code-comment: ' .. p.fg2 .. ';',
    '  --code-function: ' .. (profile and profile.syntax.call.fg or p.accent) .. ';',
    '  --code-important: ' .. p.red .. ';',
    '  --code-keyword: ' .. (profile and profile.syntax.control.fg or p.accent2) .. ';',
    '  --code-operator: ' .. p.fg1 .. ';',
    '  --code-property: ' .. (profile and (profile.syntax.property or profile.syntax.variable).fg or p.fg0) .. ';',
...
local function main()
  ...
  for _, appearance in ipairs(appearances) do
    ...
    for _, variant in ipairs({ 'dark', 'light' }) do
      ...
      files[#files + 1] = gen_apple_terminal(p, variant, term, appearance)
